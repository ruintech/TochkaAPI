...
**Язык.** Комментарии и javadoc в коде — на английском. Русским остаётся текст, пришедший из
`swagger.json` (заголовки полей, описания методов, примеры) — генератор копирует его как есть,
а также сообщения исключений, README и этот файл. При правке генератора держите это разделение:
своя обвязка в шаблонах — по-английски, подстановки из спеки — без перевода.
...
Адреса публикации заданы свойствами в `pom.xml` (`nexus.base.url` →
`https://maven.ruintech.net/repository`); при переезде Nexus правится pom, разово можно
переопределить через `-Dnexus.base.url=...`. Учётные данные берутся из секретов
`NEXUS_USERNAME` и `NEXUS_PASSWORD` и подставляются в `.mvn/ci-settings.xml` через `${env.*}` —
на диск не пишутся.
