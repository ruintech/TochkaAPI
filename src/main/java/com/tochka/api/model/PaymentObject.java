...
public enum PaymentObject {
    ...
    @JsonCreator
    public static PaymentObject fromValue(String value) {
        if (value == null) {
            return null;
        }
        for (PaymentObject candidate : values()) {
    ...
    public static PaymentObject parse(String value) {
        PaymentObject parsed = fromValue(value);
        if (parsed == null) {
            throw new IllegalArgumentException(
                    "Неизвестное значение PaymentObject: " + value);
        }
        return parsed;
    }
