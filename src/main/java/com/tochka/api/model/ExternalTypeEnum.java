...
public enum ExternalTypeEnum {
    ...
    @JsonCreator
    public static ExternalTypeEnum fromValue(String value) {
        if (value == null) {
            return null;
        }
        for (ExternalTypeEnum candidate : values()) {
    ...
    public static ExternalTypeEnum parse(String value) {
        ExternalTypeEnum parsed = fromValue(value);
        if (parsed == null) {
            throw new IllegalArgumentException(
                    "Неизвестное значение ExternalTypeEnum: " + value);
        }
        return parsed;
    }
